    ...
    <div class="col-md-12">
        <div class="card">    
            <div class="card-header">
                <div class="row">
                    <div class="col-md-12">
                        <a href="{{route('addpdf')}}"type="button" class="btn btn-md btn-outline-primary">Ajouter un nouvel offre a partir d'un pdf</a>
                    </div>
                </div>
            ...
            <div class="card-content table-responsive table-full-width">
                <table class="table">
                    <thead>
                        ...
                        <tbody>
                            @foreach($filepdfzer as $filepdf)
                                        <tr>
                                            <td>{{$filepdf->name}}</td>
                                            <td class="td-actions text-right">                                               
                                                <form action="{{ route('destroypdf', ['id'=> $filepdf->id])}}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button class="btn btn-danger btn-simple btn-xs" type="submit">Supprimer</button>
                                                </form>
